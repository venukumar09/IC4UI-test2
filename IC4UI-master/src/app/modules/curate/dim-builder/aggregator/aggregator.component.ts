...
export class AggregatorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.src.data != undefined) {
      this.src.data.attributes.map(x => ({ ...x, checked: false, key_checked : '' }));
    }
    if (this.aggregator != undefined && this.aggregator.data !== undefined) {
      const data = this.aggregator.data 
      this.aggregator_data = data;
      this.targetAttributes = data.attributes;
      this.src.data.attributes.map(x => ({ ...x, checked: false, key_checked: '' }));

      if (this.aggregator_data.keys.length > 0) {
        for (let i = 0; i < this.src.data.attributes.length; i++) {
          let attr = this.src.data.attributes[i];
          for (let j = 0; j < this.aggregator_data.keys.length; j++) {
            let key = this.aggregator_data.keys[j];
            if (key == attr.assetName) {
              this.src.data.attributes[i].key_checked = true;
            }
          }
        }
      }
  ...
  submit() {
    //const aggDetails = {aggData: this.aggregator_data , src: this.src };
    this.aggregator_data.attributes = this.targetAttributes;
    this.aggregator_data.name = this.aggregator.name;
    this.aggregator_data.source = this.src;
    const aggDetails = this.aggregator_data;
    this.sourceStatus.emit({ data: aggDetails, update: true });
  }

  checkValue(eve, itm) {
    if (eve.currentTarget.checked) {
      this.aggregator_data.attributes.push(itm);
    }
  ...
  onSrcKeyChange(eve: any, itm) {
    ...
    if (eve) {
      this.aggregator_data.keys.push(itm.assetName);
    } else {
      this.aggregator_data.keys = this.aggregator_data.keys.filter(x => x != itm.assetName);
    }
